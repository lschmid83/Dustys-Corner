...
angular.module('ngCart', ['ngCart.directives'])
    ...
    .service('ngCart', ['$rootScope', '$window', 'ngCartItem', 'store', function ($rootScope, $window, ngCartItem, store) {
		...
		// Fix: Add option parameter
        this.addItem = function (id, name, price, quantity, data, option) {
		
			// Fix: Check if items in cart have id and selected option
            var inCart = this.getItemByIdOption(id, option);

            if (typeof inCart === 'object'){
                //Update quantity of an item if it's already in the cart
                inCart.setQuantity(inCart._quantity + quantity, false);
    ...
    .factory('ngCartItem', ['$rootScope', '$log', function ($rootScope, $log) {
		...
		item.prototype.getCheckoutOption = function(){
            if(this._option.length === 0)
				return '';
			else 		
				return ' - ' + this._option + '';
        }
		...
		item.prototype.getSummaryId = function(){
        ...
        return {
            ...
            link:function(scope, element, attrs){
                ...
                scope.inCart = function(){
                    // Fix: Add Option parameter
					return  ngCart.getItemByIdOption(attrs.id, attrs.option)
                };

				// Fix: Keep quantity between 1-5.
                scope.q = 1;
                scope.qtyOpt =  [];
                for (var i = 1; i <= 5; i++) {
                    scope.qtyOpt.push(i);
        ...
        return {
            ...
            controller : ('CartController', ['$rootScope', '$scope', 'ngCart', 'fulfilmentProvider', function($rootScope, $scope, ngCart, fulfilmentProvider) {
                ...
                $scope.checkout = function () {
                    ...
                    // Fix: https://github.com/snapjay/ngCart/issues/67
